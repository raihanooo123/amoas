...
@section('content')
    <div class="auth-wrapper">
        <div class="auth-card">
            ...
            <h1 class="auth-title">{{ __('passwords.reset_title') }}</h1>

            <form method="post" action="{{ route('password.request') }}" class="auth-form" novalidate>
                @csrf
                ...
                <div class="field{{ $errors->has('email') ? ' has-error' : '' }}">
                    ...
                    <input type="email" class="form-control modern-input" id="email" name="email"
                        placeholder="{{ __('auth.email_placeholder') }}" value="{{ $email }}" autocomplete="email"
                        required>
                    @if ($errors->has('email'))
                ...
                <div class="field{{ $errors->has('password') ? ' has-error' : '' }}">
                    <label for="password">{{ __('passwords.new_password_placeholder') }}</label>
                    <input type="password" class="form-control modern-input" id="password" name="password"
                        placeholder="{{ __('passwords.new_password_placeholder') }}" autocomplete="new-password" required>
                ...
                <div class="field{{ $errors->has('password_confirmation') ? ' has-error' : '' }}">
                    <label for="password-confirm">{{ __('passwords.repeat_placeholder') }}</label>
                    <input id="password-confirm" type="password" class="form-control modern-input"
                        placeholder="{{ __('passwords.repeat_placeholder') }}" name="password_confirmation"
